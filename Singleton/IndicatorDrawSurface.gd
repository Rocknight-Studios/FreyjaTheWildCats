...
func draw_local_axis_arrows():
	if show_node_info.full_selected_path != "":
		var visual_debugger_camera = Global.visual_debugger.debugger_camera # For speed and convenience.
		...
		if node != null:
			...
			draw_line(center_of_the_node, y_arrow_tip_position, Color (y_color.r, y_color.g, y_color.b, 1.0 if mouse_is_over_arrow == VD_Mouse_is_over_arrow.Y_ARROW else y_color.a), 5.0, true)
			...
			draw_line(center_of_the_node, x_arrow_tip_position, Color (x_color.r, x_color.g, x_color.b, 1.0 if mouse_is_over_arrow == VD_Mouse_is_over_arrow.X_ARROW else x_color.a), 5.0, true)
