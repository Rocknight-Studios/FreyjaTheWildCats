...
func manage_mouse_zoom_state(direction):
	var zoom_step = ZOOM_COEFFICIENT * direction # For speed and convenience.
	var previous_zoom = zoom # To calculate the relative zoom change.
	zoom = Vector2(max(zoom.x + zoom_step, MIN_ZOOM_THRESHOLD), max(zoom.y + zoom_step, MIN_ZOOM_THRESHOLD))
