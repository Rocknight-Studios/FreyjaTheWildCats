...
func add_a_watch():
		if watch_name.text != "Type watch name":
				...
				set_item_metadata(get_item_count() - 1, [outliner.get_selected().get_metadata(0), watch_name.text])
				select(get_item_count() - 1)
				set_watch_value()

func set_watch_value():
		var current_node = get_node(get_item_metadata(get_selected_items()[0])[0]) # For speed and convenience.
		var current_node_path = current_node.get_path() # For speed and convenience.
		set_item_text(get_selected_items()[0], str(current_node_path.get_name(current_node_path.get_name_count() - 1), "->", get_item_metadata(get_selected_items()[0])[1], " = ", current_node.get(get_item_metadata(get_selected_items()[0])[1]), " : ", type_names [typeof(current_node.get(get_item_metadata(get_selected_items()[0])[1]))]))
...
func modify_watch_value():
		...
		if type_index_of_the_new_value == TYPE_NIL:
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], null)
		elif type_index_of_the_new_value == TYPE_BOOL:
				if watch_value.text.to_upper() == "TRUE":
						get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], true)
				elif watch_value.text.to_upper() == "FALSE":
						get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], false)
		elif type_index_of_the_new_value == TYPE_INT:
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], int(watch_value.text))
		elif type_index_of_the_new_value == TYPE_REAL:
				# LOG AT BOTTOM print("Conversion is performed using float constructor!")
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], float(watch_value.text))
		elif type_index_of_the_new_value == TYPE_STRING:
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], watch_value.text)
		elif type_index_of_the_new_value == TYPE_VECTOR2:
				value_array = watch_value.text.replace("(", "").replace(")", "").split(",")
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], Vector2(value_array[0], value_array[1]))
				...
				value_array = watch_value.text.replace("(", "").replace(")", "").split(",")
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], Vector3(value_array[0], value_array[1], value_array[2]))
				...
				value_array = watch_value.text.replace("(", "").replace(")", "").split(",")
				get_node(get_item_metadata(get_selected_items()[0])[0]).set(get_item_metadata(get_selected_items()[0])[1], Color(value_array[0], value_array[1], value_array[2], value_array[3]))
		...
		if get_item_metadata(get_selected_items()[0])[1] == "name":
				var full_path = get_item_metadata(get_selected_items()[0])[0] # For speed and convenience.
				...
				var selected_item_index = get_selected_items()[0] # To know, where to reset the selection.
				for i in range(0, get_item_count()):
						select(i, true)
						if get_item_metadata(get_selected_items()[0])[0] == full_path:
								set_item_metadata(get_selected_items()[0], [new_path, get_item_metadata(get_selected_items()[0])[1]])
								set_watch_value()
				select(selected_item_index, true)
